    ...
    prepare(query: string) {
      const execute = {
        async all<T>() {
          ...
          if (query.includes("grade_score, grade_letter, rn_hours_per_resident_day") && query.includes("FROM facilities WHERE state = ?") && query.includes("ORDER BY grade_score DESC LIMIT ?")) {
