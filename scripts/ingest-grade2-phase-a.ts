...
async function fetchAll(datasetId: string, label: string): Promise<CmsRow[]> {
  ...
  while (first.length === PAGE_SIZE) {
    const page = await fetchPage(datasetId, offset, label);
...
function writeShards(
  ...
  for (let start = 0, fileIndex = 1; start < values.length; start += rowsPerFile, fileIndex++) {
    ...
    const sql: string[] = [];
    if (start === 0) sql.push(`DELETE FROM ${table};`);
    ...
    const file = `scripts/${prefix}_${String(fileIndex).padStart(3, "0")}.sql`;
    writeFileSync(file, `${sql.join("\n\n")}\n`);
    files.push(file);
  }

  console.log(`${table}: wrote ${values.length.toLocaleString()} rows across ${files.length} shard(s)`);
  return files;
}
